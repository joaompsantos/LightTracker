...
{
  /*Channel mapping*/
	  int right,rightRef;//feed from right diode
    int left,leftRef ; //feed from left diode
    int threshold;  //limit
    initPIC32 ();
    ...
    while(TRUE){
      ...
      if(START==1){
        ...
        blinkLeds(1); //this function turn on the on by on, when over the robot is ready to follow the light
